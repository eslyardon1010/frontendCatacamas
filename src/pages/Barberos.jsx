...
const Barberos = () => {
	...
	const onClickAddBarbero = async e => {
		...
		try {
			await axios.post('backend-production-3de4.up.railway.app/admin/barberos/agregar', barbero)
			limpiarCampos()
			getBarberos()
			cerrarVentanaModal()
			Swal.fire(
	...
	const onClickUpdateBarbero = async e => {
		...
		try {
			await axios.put(
				'backend-production-3de4.up.railway.app/admin/barberos/' + barbero.idUsuario,
				barbero
			)
			limpiarCampos()
			getBarberos()
		...
		if (isConfirmed) {
			try {
				const res = await axios.delete(
					'backend-production-3de4.up.railway.app/admin/barberos/' + id
				)
